...
<Card dis-hover class="control-card-group h100">
	<Card dis-hover class="control-log">
		...
		<div class="">
			<Table border stripe size="small" :height="tableSize.height" :columns="controlColumns" @on-row-click="showdata" :data="controlData"></Table>
		</div>
	</Card>
...
<script>
	//发送指令
	import confirmSender from './confirm-sender.vue';
    export default {
		components: {
			'confirm-sender': confirmSender
		},
        data () {
            return {
		...
		methods: {
			...
			showdata: function(data) {
				this.$bus.emit("comfirmSender",data);
			}
		},
		watch: {
